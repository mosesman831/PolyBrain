...
- You want different models for different subtask types.
...
| `settings.timeout_sec` | 300 | Per-task timeout |
...
Validate config, then test with a simple objective:
```bash
python scripts/validate_config.py
echo "Summarize Apple's latest quarterly earnings in 3 bullets with sources" | python scripts/orchestrate.py
```